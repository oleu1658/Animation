...
Declare_Any_Class( "Windmill",          // As our shapes get more complicated, we begin using matrices and flow control (including loops) to
  ...
  // ******** WALL WITH WINDOW
Declare_Any_Class( "Wall_With_Window",   
  { populate()  
	{
	this.positions.push(vec3(-5,0,0));
	this.positions.push(vec3(5,0,0));
	this.positions.push(vec3(-5,1.4,0));
	this.positions.push(vec3(-1.6,1.4,0));
	this.positions.push(vec3(1.6,1.4,0));
	this.positions.push(vec3(5,1.4,0));
	this.positions.push(vec3(-5,3,0));
	this.positions.push(vec3(-1.6,3,0));
	this.positions.push(vec3(1.6,3,0));
	this.positions.push(vec3(5,3,0));
	this.positions.push(vec3(-5,3.8,0));
	this.positions.push(vec3(5,3.8,0));

	this.positions.push(vec3(-5,0,-1));
	this.positions.push(vec3(5,0,-1));
	this.positions.push(vec3(-5,1.4,-1));
	this.positions.push(vec3(-1.6,1.4,-1));
	this.positions.push(vec3(1.6,1.4,-1));
	this.positions.push(vec3(5,1.4,-1));
	this.positions.push(vec3(-5,3,-1));
	this.positions.push(vec3(-1.6,3,-1));
	this.positions.push(vec3(1.6,3,-1));
	this.positions.push(vec3(5,3,-1));
	this.positions.push(vec3(-5,3.8,-1));
	this.positions.push(vec3(5,3.8,-1));
...
//
	...
	this.texture_coords.push([0,1.4/3.8]);
	this.texture_coords.push([3.4/10,1.4/3.8]);
	this.texture_coords.push([6.6/10,1.4/3.8]);
	this.texture_coords.push([1,1.4/3.8]);
	this.texture_coords.push([0,3/3.8]);
	this.texture_coords.push([3.4/10,3/3.8]);
	this.texture_coords.push([6.6/10,3/3.8]);
	this.texture_coords.push([1,3/3.8]);
	this.texture_coords.push([0,1]);
	this.texture_coords.push([1,1]);
	...
	for(i = 0; i <= 12; i+=12)
	{
		this.indices.push(i+0,i+1,i+2, i+1,i+2,i+5);
		this.indices.push(i+2,i+3,i+6, i+3,i+6,i+7);
		this.indices.push(i+4,i+5,i+8, i+5,i+8,i+9);
		this.indices.push(i+6,i+9,i+10, i+9,i+10,i+11);
	}
	this.indices.push(24,25,26, 25,26,27);
	this.indices.push(28,29,30, 29,30,31);
	this.indices.push(32,33,34, 33,34,35);
	this.indices.push(36,37,38, 37,38,39);
	this.indices.push(40,41,42, 41,42,43);
	this.indices.push(44,45,46, 45,46,47);
	this.indices.push(48,49,50, 49,50,51);
	this.indices.push(52,53,54, 53,54,55);
	}
 }, Shape )
 

  // ******** CUPCAKE *********
Declare_Any_Class( "Cupcake",
  { 'populate'( num_blades )
      {
	  for( var i = 0; i < num_blades; i++ )
	  {
		var spin = rotation(i * 360/num_blades, 0, 1, 0 );
		var spin2 = rotation((i+1) * 360/num_blades, 0, 1, 0 );
		var newPoint = mult_vec( spin, vec4( 1, 0, 0, 1 ) );
		var newPoint2 = mult_vec( spin2, vec4( 1, 0, 0, 1 ) );
		var newPoint3 = mult_vec( spin, vec4( .7, 0, 0, 1 ) );
		var newPoint4 = mult_vec( spin2, vec4( .7, 0, 0, 1 ) );
		this.positions.push( vec3 ( 0, 0, 0 ));
		this.positions.push( vec3 ( newPoint3[0], 0, newPoint3[2] ) );
		this.positions.push( vec3 ( newPoint4[0], 0, newPoint4[2] ) );
		this.positions.push( vec3 ( 0, 1.7, 0 ) );
		this.positions.push( vec3 ( newPoint[0], 1.2, newPoint[2] ) );
		this.positions.push( vec3 ( newPoint2[0], 1.2, newPoint2[2] ) );
		this.positions.push( vec3 ( newPoint3[0], 0, newPoint3[2] ) );
		this.positions.push( vec3 ( newPoint[0], 1.2, newPoint[2] ) );
		this.positions.push( vec3 ( newPoint4[0], 0, newPoint4[2] ) );
		this.positions.push( vec3 ( newPoint2[0], 1.2, newPoint2[2] ) );
	
		var newNormal1 = mult_vec( spin, vec4( 0, -1, 0, 0 ) );
		this.normals.push( newNormal1.slice(0,3) );
		this.normals.push( newNormal1.slice(0,3) );	
		this.normals.push( newNormal1.slice(0,3) );	

		var newNormal2 = mult_vec( spin, vec4( .5*Math.sin(2*Math.PI*num_blades/360), Math.sin(2*Math.PI*num_blades/360), -.5+.5*Math.cos(2*Math.PI*num_blades/360), 0 ) );
		this.normals.push( newNormal2.slice(0,3) );
		this.normals.push( newNormal2.slice(0,3) );
		this.normals.push( newNormal2.slice(0,3) );

		var newNormal3 = mult_vec( spin, vec4( (1.2*Math.sin(2*Math.PI*num_blades/360), -.3*Math.sin(2*Math.PI*num_blades/360),1.2*(Math.cos(2*Math.PI*num_blades/360)-1)), 0 ) );
		this.normals.push( newNormal3.slice(0,3) );
		this.normals.push( newNormal3.slice(0,3) );
		this.normals.push( newNormal3.slice(0,3) );
		this.normals.push( newNormal3.slice(0,3) );

		this.texture_coords.push(vec2(0,0));
		this.texture_coords.push(vec2(newPoint3[0],newPoint3[2]));
		this.texture_coords.push(vec2(newPoint4[0],newPoint4[2]));
		this.texture_coords.push(vec2(0,0));
		this.texture_coords.push(vec2(newPoint[0],newPoint[2]));
		this.texture_coords.push(vec2(newPoint2[0],newPoint2[2]));
		this.texture_coords.push(vec2(i*360/num_blades,0));
		this.texture_coords.push(vec2(i*360/num_blades,1));
		this.texture_coords.push(vec2((i+1)*360/num_blades,0));
		this.texture_coords.push(vec2((i+1)*360/num_blades,1));

		
		this.indices.push(10*i);
		this.indices.push(10*i+1);
		this.indices.push(10*i+2);

		this.indices.push(10*i+3);
		this.indices.push(10*i+4);
		this.indices.push(10*i+5);

		this.indices.push(10*i+6);
		this.indices.push(10*i+7);
		this.indices.push(10*i+8);
		this.indices.push(10*i+7);
		this.indices.push(10*i+8);
		this.indices.push(10*i+9);	
	   }
      }
  }, Shape )
