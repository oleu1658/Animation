...
Declare_Any_Class( "Example_Animation",  // An example of a Scene_Component that our class Canvas_Manager can manage.  This one draws the scene's 3D shapes.
  ...
  // *******************************************************************
  // Assignment 2
Declare_Any_Class( "Hogwarts_Studies",
  { 'construct'( context )
	{ var shapes = { "cupcake" : new Cupcake()};
	  this.submit_shapes( context, shapes);
    ...
    'display'( graphics_state )
	{
	  ...
	  this.shapes.cupcake .draw(graphics_state, model_transform, this.yellow_clay); 
	}
  }, Scene_Component );
...
Declare_Any_Class( "Hogwarts_Studies",  // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
  { 'construct'( context )
      { this.globals = context.globals;
        var shapes = { "box" : new        Cube(), 
                       "ball": new Grid_Sphere( 30, 30),
			"north_wall": new Wall_With_Window(),
			"wing": new Rounded_Capped_Cylinder(30,30),
			"cupcake": new Cupcake(100),
			"candlestick": new Capped_Cylinder(30,30),
			"wand": new Closed_Cone(30,30),
			"dialogue_text": new Text_Line(48) }; 
        this.submit_shapes( context, shapes );
        
        this.define_data_members({ black: context.shaders_in_use["Phong_Model"].material(Color(0,0,0,1),.5,.5,.5,40), 
				butterfly_wing: context.shaders_in_use["Fake_Bump_Map"].material(Color(.78,.54,.88,1),.5,.5,.5,40, context.textures_in_use["butterfly.jpg"]),
				wand_texture: context.shaders_in_use["Phong_Model"].material(Color(.7,.58,.45,1),.5,.5,.5,40, context.textures_in_use["wand_texture.jpg"]),
				cupcake_texture: context.shaders_in_use["Fake_Bump_Map"].material(Color(.76,.65,.37,1),.5,.2,.2,40, context.textures_in_use["cupcake.jpg"]),
				pink_cupcake_texture: context.shaders_in_use["Fake_Bump_Map"].material(Color(.99,.75,.92,1),.5,.2,.2,40, context.textures_in_use["cupcake.jpg"]),
				sky: context.shaders_in_use["Phong_Model"].material(Color(.67,.85,.95,1),.2,1,.5,40, context.textures_in_use["sky.jpg"]),
				brick: context.shaders_in_use["Phong_Model"].material(Color(.55,.34,.28,1),.2,1,1,40, context.textures_in_use["bricks.jpg"]),
				tile: context.shaders_in_use["Phong_Model"].material(Color(.57,.56,.55,1),.2,1,1,40, context.textures_in_use["tile.jpg"]),
				ceiling: context.shaders_in_use["Phong_Model"].material(Color(.52,.47,.43,1),.2,.5,.5,40, context.textures_in_use["ceiling.jpg"]),
				wood: context.shaders_in_use["Phong_Model"].material(Color(.37,.17,.09,1),.2,1,1,40, context.textures_in_use["wood.jpg"]),
				candle: context.shaders_in_use["Phong_Model"].material(Color(.95,.95,.95,1),.2,1,1,40, context.textures_in_use["candle.jpg"]),
				blue_candle: context.shaders_in_use["Phong_Model"].material(Color(.64,.99,.98,1),.2,1,1,40, context.textures_in_use["candle.jpg"]),
				fire: context.shaders_in_use["Fake_Bump_Map"].material(Color(.99,.81,.29,1),.5,.2,.2,40, context.textures_in_use["fire.jpg"]),
				stone: context.shaders_in_use["Fake_Bump_Map"].material(Color(.65,.64,.62,1),.5,.2,.2,40, context.textures_in_use["stone.jpg"]),
    ...
    'display'( graphics_state )
      { 
	// *** Lights ***
	graphics_state.lights = [ new Light ( vec4(  0,  34, 7, 1 ), Color( .99, .99, .71, 1 ), 30000 ), new Light (vec4(30,30,34,1), Color(.99,.99,.71,1), 1000)];

	
	var timeSinceSceneStart = graphics_state.animation_time/1000,
	   font_scale = scale(.02,.04,1),
	   model_transform = mult(translation(-.5,.5,0),font_scale),
	   dialogue_strings = ["Time to start studying for my O.W.L.'s.",
				"Colovaria!",
				"Diminuendo!",
				"Engorgio!",
				"Duro!",
				...
				"Ah well. Guess that means I can't study anymore.",
				"What a tragedy."],
	...
	for(i = 0; i < 16; i++)
	{ 
		if(i%2 == 0 && i < 13 && timeSinceSceneStart > i*2 && timeSinceSceneStart < (i*2+2)) 
		{
			this.shapes.dialogue_text.set_string(dialogue_strings[i/2]);
			this.shapes.dialogue_text.draw(this.graphics_state,model_transform,this.text_material);
		}
		if(i > 13 && timeSinceSceneStart > 13*2+(i-13)*3 && timeSinceSceneStart < 13*2+(i-12)*3)
		{
			this.shapes.dialogue_text.set_string(dialogue_strings[Math.round(i/2.)]);
			this.shapes.dialogue_text.draw(this.graphics_state,model_transform,this.text_material);
		}
	}
	
	// Camera angle
	graphics_state.camera_transform = lookAt(vec3(0,6,26),vec3(0,0,0),vec3(0,1,0));
	if(timeSinceSceneStart >= 23 && timeSinceSceneStart <= 23.5)
		graphics_state.camera_transform = lookAt(vec3(0,6,26),vec3(1.5*((t-23)/0.5),3*((t-23)/0.5),-9*(t-23)*(t-23)/0.5),vec3(0,1,0));
	if(timeSinceSceneStart >= 23.5 && timeSinceSceneStart <= 29)
		graphics_state.camera_transform = lookAt(vec3(1.5*(t-23.5)/5.5,6-3*(t-23.5)/5.5,26-(11+9*(t-23))*(t-23.5)/5.5),vec3(1.5,3,-9*(t-23)),vec3(0,1,0));
	if(timeSinceSceneStart >= 29)
		graphics_state.camera_transform = lookAt(vec3(1.5,3,-39),vec3(1.5,3,-9*(t-23)),vec3(0,1,0));

	// Cupcake
	model_transform = identity();
	if(timeSinceSceneStart < 6)
		 this.shapes.cupcake .draw(graphics_state, model_transform, this.pink_cupcake_texture);
	if(timeSinceSceneStart >= 10 && timeSinceSceneStart <= 11) // Diminuendo = decrease size
		model_transform = mult(model_transform, scale(1-.5*(t-10),1-.5*(t-10),1-.5*(t-10)));
	if(timeSinceSceneStart >= 11 && timeSinceSceneStart <= 14)
		model_transform = mult(model_transform, scale(.5,.5,.5));
	if(timeSinceSceneStart >= 14 && timeSinceSceneStart <= 15) // Engorgio = increase size
		model_transform = mult(model_transform, scale(.5+1*(t-14),.5+1*(t-14),.5+1*(t-14)));	
	if(timeSinceSceneStart >=15)
		model_transform = mult(model_transform, scale(1.5,1.5,1.5));
	if(timeSinceSceneStart >= 6 && timeSinceSceneStart < 18) // Colovaria = change color 
		this.shapes.cupcake .draw(graphics_state, model_transform, this.cupcake_texture);
	if(timeSinceSceneStart >= 22 && timeSinceSceneStart < 23) // Spell causes cupcake to shoot up and rotate
		model_transform = mult(mult(mult(model_transform,translation(3,0,0)),rotation(-25*(t-22),[0,0,1])),translation(-3,0,0));
	if(timeSinceSceneStart >= 18 && timeSinceSceneStart < 23) // Duro = change to stone
		this.shapes.cupcake .draw(graphics_state,model_transform,this.stone);
		
	// Candle
	if(timeSinceSceneStart >= 22 && timeSinceSceneStart < 23)
		model_transform = mult(mult(mult(model_transform,translation(.5,1.5,0)), rotation(-35*(t-22),[0,0,1])),translation(-.5,-1.5,0));
	model_transform_candlestick = mult(mult(model_transform,translation(0,2,0)),rotation(90,[1,0,0]));	
	if(timeSinceSceneStart < 6)
	{
		this.shapes.candlestick .draw(graphics_state, mult(model_transform_candlestick,scale(.05,.05,1)), this.candle);
		this.shapes.ball .draw(graphics_state,mult(mult(model_transform,translation(0,2.55,0)),scale(.1,.1,.1)), this.fire);
	}
	if(timeSinceSceneStart >= 6 && timeSinceSceneStart < 18)
	{
		this.shapes.candlestick .draw(graphics_state, mult(model_transform_candlestick,scale(.05,.05,1)), this.blue_candle);
		this.shapes.ball .draw(graphics_state,mult(mult(model_transform,translation(0,2.55,0)),scale(.1,.1,.1)), this.fire);
	}
	if(timeSinceSceneStart >= 18 && timeSinceSceneStart < 23)
	{
		this.shapes.candlestick .draw(graphics_state, mult(model_transform_candlestick,scale(.05,.05,1)), this.stone);
		this.shapes.ball .draw(graphics_state,mult(mult(model_transform,translation(0,2.55,0)),scale(.1,.1,.1)), this.stone);	
	}
	
	// Butterfly
	model_transform = identity();
	model_transform = mult(mult(mult(mult(model_transform,translation(1.5,3,0)),translation(Math.sin(2*(t-23)*Math.PI),Math.abs(Math.cos(2*(t-23)*Math.PI)),-9*(t-23))),rotation(20,[1,0,0])),rotation(90,[0,1,0])); 
	if(timeSinceSceneStart > 23)
	{
		this.shapes.ball .draw(graphics_state,mult(model_transform,scale(.75,.25,.25)), this.black);	
		this.shapes.ball .draw(graphics_state,mult(mult(model_transform,translation(.49,0,0)),scale(.25,.25,.25)), this.black);
		var model_transform_right = mult(mult(mult(mult(model_transform,translation(0,.24,0)),rotation(90,[1,0,0])),rotation(-45*Math.cos(4*(t-23)*Math.PI),[1,0,0])),translation(0,.5,0));
		var model_transform_left = mult(mult(mult(mult(model_transform,translation(0,.24,0)),rotation(-90,[1,0,0])),rotation(45*Math.cos(4*(t-23)*Math.PI),[1,0,0])),translation(0,.5,0));
		this.shapes.wing .draw(graphics_state,mult(mult(model_transform_right,translation(.35,0,0)),scale(.5,.5,.01)), this.butterfly_wing);
		this.shapes.wing .draw(graphics_state,mult(mult(model_transform_right,translation(-.35,0,0)),scale(.5,.5,.01)), this.butterfly_wing);
		this.shapes.wing .draw(graphics_state,mult(mult(model_transform_left,translation(.35,0,0)),scale(.5,.5,.01)), this.butterfly_wing);
		this.shapes.wing .draw(graphics_state,mult(mult(model_transform_left,translation(-.35,0,0)),scale(.5,.5,.01)), this.butterfly_wing);
	}
	...
	for(i = 1; i < 6; i++)
	{
		if(timeSinceSceneStart >= i*4 && timeSinceSceneStart <= i*4+1)
			model_transform = mult(mult(mult(mult(mult(mult(mult(model_transform,translation(.5,3.5,22)),rotation(45,[1,0,0])),translation(2*Math.sin(.61),0,2*Math.cos(.61))),rotation(-180*(t-i*4),[-1*Math.tan(.35),0,-1])),translation(-2*Math.sin(.61),0,-2*Math.cos(.61))), rotation(-45,[1,0,0])),translation(-.5,-3.5,-22));
		if(timeSinceSceneStart >= i*4+1 && timeSinceSceneStart <= i*4+2)
			model_transform = mult(mult(mult(mult(mult(mult(mult(mult(mult(mult(model_transform,translation(.5,3.5,22)),rotation(45,[1,0,0])),translation(2*Math.sin(.61),0,2*Math.cos(.61))),rotation(5,[0,1,0])),rotation(-30*(t-(i*4+1)),[1,0,0])),rotation(-5,[0,1,0])),rotation(-180,[-1*Math.tan(.35),0,-1])),translation(-2*Math.sin(.61),0,-2*Math.cos(.61))), rotation(-45,[1,0,0])),translation(-.5,-3.5,-22));
		if(timeSinceSceneStart >= i*4+2 && timeSinceSceneStart <= i*4+4)
			model_transform = mult(mult(mult(mult(mult(mult(mult(mult(mult(mult(mult(model_transform,translation(.5,3.5,22)),rotation(45,[1,0,0])),translation(2*Math.sin(.61),0,2*Math.cos(.61))),rotation(5,[0,1,0])),rotation(21.97*(t-(i*4+2)),[Math.sin(.52)*Math.cos(.61),Math.cos(.52)*Math.sin(.61),-1*Math.sin(.52)*Math.sin(.61)])),rotation(-30,[1,0,0])),rotation(-5,[0,1,0])),rotation(-180,[-1*Math.tan(.35),0,-1])),translation(-2*Math.sin(.61),0,-2*Math.cos(.61))), rotation(-45,[1,0,0])),translation(-.5,-3.5,-22));	
	}	
	model_transform = mult(mult(mult(model_transform,translation(.5,3.5,22)),rotation(-135,[1,0,0])),rotation(-35,[0,1,0]));
	this.shapes.wand .draw(graphics_state,mult(model_transform,scale(.1,.1,2)),this.wand_texture);

	// Desk
	model_transform = mult(identity(), translation(0,-1,0));
	this.shapes.box .draw(graphics_state,mult(model_transform, scale(10,1,6)), this.wood);
	model_transform = mult(identity(),scale(1,6,1));
	this.shapes.box .draw(graphics_state,mult(model_transform, translation(-9,-4./3,5)), this.wood);
	this.shapes.box .draw(graphics_state,mult(model_transform, translation(-9,-4./3,-5)), this.wood);
	this.shapes.box .draw(graphics_state,mult(model_transform, translation(9,-4./3,5)), this.wood);
	this.shapes.box .draw(graphics_state,mult(model_transform, translation(9,-4./3,-5)), this.wood);

	// Floor, Roof
	model_transform = mult(identity(), scale(50,.5,50));
	this.shapes.box .draw(graphics_state,mult(model_transform, translation(0,-29,0)), this.tile);
	this.shapes.box .draw(graphics_state,mult(model_transform, translation(0,49,0)), this.ceiling);

	// North Wall (with window)
	model_transform = mult(identity(), scale(10,10,1));
	this.shapes.north_wall .draw(graphics_state,mult(model_transform, translation(0,-1.4,-50)), this.brick);
	
	// Walls
	model_transform = mult(identity(), scale(.5,19,50));
	this.shapes.box .draw(graphics_state,mult(model_transform, translation(-101,5./19,0)), this.brick);
	this.shapes.box .draw(graphics_state,mult(model_transform, translation(101,5./19,0)), this.brick);
	model_transform = mult(identity(), scale(50,19,.5));
	this.shapes.box .draw(graphics_state,mult(model_transform, translation(0,5./19,101)), this.brick);

	// Sky
	graphics_state.lights = [new Light(vec4(30,30,34,1), Color(.99,.99,.66,1),50000)];
	model_transform = mult(mult(identity(), scale(150,150,150)),rotation(-135,[1,0,0]));
	this.shapes.ball .draw(graphics_state,model_transform, this.sky);
...
Declare_Any_Class( "Flag_Toggler",  // A class that just interacts with the keyboard and reports strings
  { 'construct'( context ) { this.globals    = context.globals; },
	...
	user_interface_string_manager.string_map["frame_rate"] = "Frame Rate: " + Math.round(this.globals.graphics_state.animation_delta_time/10) + " fps";
      },
  }, Scene_Component );
